...
// Particle constellation with organic movement and scroll wave
function OrganicParticles({ color = '#00ffff', scrollProgress = 0 }: { color: string, scrollProgress?: number }) {
  const groupRef = useRef<THREE.Group>(null)
  ...
  useFrame((state) => {
    if (groupRef.current) {
      // Color spreading animation for particles
      if (isSpreading && spreadProgress < 1) {
        const newProgress = spreadProgress + 0.008 // Faster for particles
        setSpreadProgress(newProgress)
        
        // Update particle colors with artistic random spreading
        const newColor = new THREE.Color(currentColor)
        const oldColor = new THREE.Color(previousColor)
        
        // Create artistic spreading pattern for particles
        const time = state.clock.elapsedTime
        const spreadRadius = spreadProgress * 12 // Much larger radius to ensure all particles are covered
        ...
        particles.forEach((particle, index) => {
          if (particle.material) {
            ...
            // Create multiple spreading centers with organic movement
            const centers = [
              { x: Math.sin(time * 0.4) * 3, y: -4 + spreadProgress * 8, z: Math.cos(time * 0.2) * 2 },
              { x: Math.cos(time * 0.6) * 2.5, y: -2 + spreadProgress * 7, z: Math.sin(time * 0.3) * 2.5 },
              { x: Math.sin(time * 0.8) * 2, y: -3 + spreadProgress * 9, z: Math.cos(time * 0.5) * 2.2 }
            ]
            
            // Calculate distance to nearest spreading center
            let minDistance = Infinity
            
            centers.forEach(center => {
              const distance = Math.sqrt(
                Math.pow(particleX - center.x, 2) + 
                Math.pow(particleY - center.y, 2) + 
                Math.pow(particleZ - center.z, 2)
              )
            ...
            // Add organic noise to the spread
            const noise = Math.sin(particleX * 2 + time) * Math.cos(particleZ * 1.5 + time * 0.4) * 0.6
            const adjustedDistance = minDistance + noise
            
            // Artistic spreading threshold with organic variation - ensure all particles are covered
            const spreadThreshold = spreadRadius + Math.sin(index * 0.05 + time) * 1.5
            
            if (adjustedDistance < spreadThreshold) {
              // Calculate artistic transition progress
              const baseProgress = Math.max(0, Math.min(1, (spreadThreshold - adjustedDistance) / 2.5))
              const artisticProgress = Math.pow(baseProgress, 0.7) // Smoother curve for better visibility
              
              // Smooth transition from old to new color
              const lerpedColor = oldColor.clone()
              lerpedColor.lerp(newColor, artisticProgress)
              ;(particle.material as THREE.MeshBasicMaterial).color = lerpedColor
            }
            // Particles outside the spread keep the old color
          }
        })
      ...
      // Organic floating movement for each sphere
      particles.forEach((particle, i) => {
        const time = state.clock.elapsedTime
        const speed = 0.001 + Math.random() * 0.002
        
        particle.position.x += Math.sin(time + i * 0.1) * speed
        particle.position.y += Math.cos(time + i * 0.15) * speed
        particle.position.z += Math.sin(time * 0.5 + i * 0.05) * speed
      ...
      // Add scroll-based wave to particle field
      const waveEffect = Math.sin(scrollProgress * Math.PI * 6) * 0.5
      groupRef.current.rotation.x = waveEffect * 0.3
      groupRef.current.rotation.z = Math.cos(scrollProgress * Math.PI * 4) * 0.2
    }
  })
...
function Scene({ currentColor, scrollProgress }: { currentColor: string, scrollProgress: number }) {
  ...
  return (
    <>
      ...
      {/* Particle field with scroll wave */}
      {/* <OrganicParticles color={currentColor} scrollProgress={scrollProgress} /> */}
    </>
  )
}
